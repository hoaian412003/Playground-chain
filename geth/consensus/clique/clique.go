...
func ConvertUint256ToEth(value *big.Int) *big.Float {
	ethValue := new(big.Float).SetInt(value)
	ethValue = ethValue.Quo(ethValue, big.NewFloat(math.Pow10(18)))
	return ethValue
}
...
func (c *Clique) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if !checkStakedSigner(signer, c) {
		return errors.New("singers must staked >= 32 ETH to seal to block")
	}
